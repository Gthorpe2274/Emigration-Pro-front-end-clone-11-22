...
// src/worker/retention-cleanup.ts
var RetentionCleanupService = class {
  static {
    __name(this, "RetentionCleanupService");
  }
  db;
  kv;
  constructor(db, kv) {
    ...
    try {
      ...
      for (const assessment of expiredAssessments.results) {
        ...
        try {
          ...
          for (const report2 of reports.results) {
            if (this.kv && report2.pdf_url) {
              try {
                await this.kv.delete(String(report2.pdf_url));
                results.kvsDeleted++;
                console.log(`Deleted PDF from KV: ${report2.pdf_url}`);
              } catch (kvError) {
                console.warn(`Failed to delete PDF from KV: ${report2.pdf_url}`, kvError);
              }
            }
          }
      ...
      console.error("Cleanup failed:", error3);
      throw error3;
    }
  }
  /**
   ...
   */
  async isAssessmentRetained(assessmentId) {
   ...
   */
  async extendRetention(assessmentId, additionalMonths = 12) {
    const result = await this.db.prepare(`
      UPDATE assessments 
...
async function runScheduledCleanup(env2) {
  console.log("=== SCHEDULED RETENTION CLEANUP STARTED ===");
  try {
    const retentionService = new RetentionCleanupService(env2.DB, env2.REPORTS_KV);
...
__name(runScheduledCleanup, "runScheduledCleanup");
...
var blogAdminAuth = /* @__PURE__ */ __name(async (c, next) => {
...
var AssessmentSchema = external_exports.object({
  user_age: external_exports.number().min(18).max(100),
  user_job: external_exports.string().min(1).max(200),
  preferred_country: external_exports.string().min(1).max(100),
  preferred_city: external_exports.string().optional(),
  location_preference: external_exports.enum(["beachside", "rural", "city"]),
  climate_preference: external_exports.enum(["tropical", "seasonal", "dry", "mediterranean", "temperate", "northern"]).optional(),
  monthly_budget: external_exports.number().min(100).max(5e4).optional().default(2e3),
  immigration_policies_importance: external_exports.number().min(1).max(5),
  healthcare_importance: external_exports.number().min(1).max(5),
  safety_importance: external_exports.number().min(1).max(5),
  internet_importance: external_exports.number().min(1).max(5),
  emigration_process_importance: external_exports.number().min(1).max(5),
  ease_of_immigration_importance: external_exports.number().min(1).max(5),
  local_acceptance_importance: external_exports.number().min(1).max(5)
});
var cityBudgetData = {
  "Portugal": {
    "Lisbon": { oneBedroomMin: 700, oneBedroomMax: 1200, twoBedroomMin: 1e3, twoBedroomMax: 1800, currency: "EUR" },
    "Porto": { oneBedroomMin: 500, oneBedroomMax: 900, twoBedroomMin: 700, twoBedroomMax: 1300, currency: "EUR" },
    "default": { oneBedroomMin: 600, oneBedroomMax: 1e3, twoBedroomMin: 800, twoBedroomMax: 1400, currency: "EUR" }
  },
  "Spain": {
    "Barcelona": { oneBedroomMin: 800, oneBedroomMax: 1500, twoBedroomMin: 1200, twoBedroomMax: 2200, currency: "EUR" },
    "Madrid": { oneBedroomMin: 750, oneBedroomMax: 1400, twoBedroomMin: 1100, twoBedroomMax: 2e3, currency: "EUR" },
  ...
  "Costa Rica": {
    "San Jos\xE9": { oneBedroomMin: 500, oneBedroomMax: 900, twoBedroomMin: 700, twoBedroomMax: 1300, currency: "USD" },
    "default": { oneBedroomMin: 400, oneBedroomMax: 700, twoBedroomMin: 600, twoBedroomMax: 1e3, currency: "USD" }
  },
  "default": {
    "default": { oneBedroomMin: 800, oneBedroomMax: 1500, twoBedroomMin: 1200, twoBedroomMax: 2200, currency: "USD" }
  }
};
function analyzeBudgetCompatibility(monthlyBudget, country, city) {
  ...
  if (cityData.currency === "EUR") {
    localBudget = monthlyBudget * 0.92;
  }
...
__name(analyzeBudgetCompatibility, "analyzeBudgetCompatibility");
function calculateScore(assessment) {
  ...
  if (climateMatchType === "complete_mismatch") {
    console.log(`\u{1F6AB} COMPLETE CLIMATE MISMATCH DETECTED: ${assessment.climate_preference} vs ${countryData.climate_type}`);
    return {
      error: true,
...
app.post("/api/assessments", zValidator("json", AssessmentSchema), async (c) => {
  try {
    ...
    const retention_expires_at = /* @__PURE__ */ new Date();
    ...
    console.error("Error creating assessment:", error3);
...
app.get("/api/assessments/:id", async (c) => {
  try {
    ...
    if (result.retention_expires_at) {
      const retentionExpiry = new Date(result.retention_expires_at);
    ...
    console.error("Error fetching assessment:", error3);
    return c.json({ error: "Failed to fetch assessment" }, 500);
  }
});
var EmailLeadSchema = external_exports.object({
  email: external_exports.string().email(),
  assessment_id: external_exports.number().optional()
});
app.post("/api/subscribe-for-permanent-access", zValidator("json", EmailLeadSchema), async (c) => {
  try {
    ...
    console.error("Error capturing email lead:", error3);
...
app.post("/api/admin/cleanup", async (c) => {
  try {
    ...
    console.error("Manual cleanup error:", error3);
    return c.json({
      success: false,
...
var PermanentAccessSchema = external_exports.object({
  assessment_id: external_exports.number(),
  email: external_exports.string().email(),
  purchase_confirmed: external_exports.boolean().default(true)
});
app.post("/api/relocation-hub/create-access", zValidator("json", PermanentAccessSchema), async (c) => {
  try {
    ...
    const generateSessionCode = /* @__PURE__ */ __name(() => {
    ...
    console.error("Error creating permanent access:", error3);
    if (error3 instanceof external_exports.ZodError) {
      return c.json({
        error: "Validation failed",
        details: error3.errors,
...
var AccessRequestSchema = external_exports.object({
  email: external_exports.string().email(),
  session_code: external_exports.string().min(1)
});
app.post("/api/relocation-hub/access", zValidator("json", AccessRequestSchema), async (c) => {
  try {
    ...
    console.error("Error accessing relocation hub:", error3);
...
app.get("/api/admin/crm/purchasers", async (c) => {
  try {
    ...
    console.error("Error fetching purchasers:", error3);
...
app.get("/api/admin/email-leads", async (c) => {
  try {
    ...
    console.error("Error fetching email leads:", error3);
...
var BlogPostSchema = external_exports.object({
  ...
  slug: external_exports.string().min(1).max(200),
  featured_image: external_exports.string().optional(),
  body: external_exports.string().min(1),
  excerpt: external_exports.string().optional(),
  published_date: external_exports.string().optional(),
  is_published: external_exports.boolean().default(false),
  allow_comments: external_exports.boolean().default(true),
...
app.get("/api/blog/posts", async (c) => {
  try {
    ...
    console.error("Error fetching blog posts:", error3);
...
app.get("/api/blog/posts/:slug", async (c) => {
  try {
    ...
    console.error("Error fetching blog post:", error3);
...
app.get("/api/admin/blog/posts", blogAdminAuth, async (c) => {
  try {
    ...
    console.error("Error fetching all blog posts:", error3);
    return c.json({ error: "Failed to fetch blog posts" }, 500);
  }
});
app.post("/api/admin/blog/posts", blogAdminAuth, zValidator("json", BlogPostSchema), async (c) => {
  try {
    ...
    console.error("Error creating blog post:", error3);
...
app.put("/api/admin/blog/posts/:id", blogAdminAuth, zValidator("json", BlogPostSchema), async (c) => {
  try {
    ...
    console.error("Error updating blog post:", error3);
...
app.delete("/api/admin/blog/posts/:id", blogAdminAuth, async (c) => {
  try {
    ...
    console.error("Error deleting blog post:", error3);
    return c.json({ error: "Failed to delete blog post" }, 500);
  }
});
async function handleGenerateImage(c) {
  const env2 = c.env;
  const body = await c.req.json().catch(() => ({}));
  const prompt = body?.prompt ?? body?.text ?? "";
  const size = body?.size ?? "1024x1024";
  const outFormat = body?.format ?? "data_url";
  if (!prompt || typeof prompt !== "string" || prompt.trim() === "") {
    return c.json({ error: "Missing 'prompt' in request body" }, 400);
  }
  const GEMINI_API_KEY = env2.GEMINI_API_KEY;
  const OPENAI_API_KEY = env2.OPENAI_API_KEY;
  if (GEMINI_API_KEY) {
    try {
      ...
      const maybeUrl = data?.candidates?.[0]?.content?.parts?.find((p) => p.url)?.url;
      if (!base64 && !maybeUrl) {
        return c.json({
...
var index_default = {
  ...
  async scheduled(event, env2, ctx) {
    if (event.cron === "0 2 * * *") {
      ...
      ctx.waitUntil(runScheduledCleanup(env2));
    }
  }
};
export {
